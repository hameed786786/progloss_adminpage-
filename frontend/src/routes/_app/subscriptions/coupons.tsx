  ...
  return (
    <>
      <TopBar title="Subscription Coupons" subtitle="Plan-level discount codes & promotions" actions={
        <PrimaryBtn onClick={()=>setOpen(true)}><Plus className="h-3.5 w-3.5"/> <span className="hidden sm:inline">New coupon</span></PrimaryBtn>
      }/>
      <div className="px-4 py-4 sm:px-6 sm:py-6 space-y-6">
        <div className="grid gap-4 md:grid-cols-4">
          <KpiCard label="Active coupons" value={String(coupons.filter(c=>c.status==="active").length)} icon={Ticket} accent="primary" />
          <KpiCard label="Total redemptions" value={String(coupons.reduce((s,c)=>s+(c.redeemed ?? 0),0))} icon={Ticket} accent="success" />
          <KpiCard label="Discount given (MTD)" value="AED 24,180" icon={Ticket} accent="warning" />
          <KpiCard label="Avg uplift" value="+18.4%" icon={Ticket} accent="success" />
        </div>
...
function CreateCouponDialog({ open, onOpenChange, onCreate, exists, plans }: { open: boolean; onOpenChange: (o:boolean)=>void; onCreate: (c: Coupon)=>void; exists: (code: string)=>boolean; plans: any[] }) {
  ...
  const submit = () => {
    ...
    const coupon: Coupon = { code: c, plan, discount, redeemed: 0, cap: parseInt(cap)||0, expires: formatted, status: "active" };
    onCreate(coupon);
